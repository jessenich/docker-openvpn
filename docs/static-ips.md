...
1. Create a client specific configuration:
        ...
        $ echo "ifconfig-push 192.168.254.1 192.168.254.2" | docker run -v $OVPN_DATA:/etc/openvpn -i --rm jessenich91/openvpn tee /etc/openvpn/ccd/CERT_COMMON_NAME
...
Login to the data volume with a `bash` container, note only changes in /etc/openvpn will persist:
    ...
    docker run -v $OVPN_DATA:/etc/openvpn -it --rm jessenich91/openvpn bash -l
