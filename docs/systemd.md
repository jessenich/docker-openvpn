...
2. Initialize the data container, but don't start the container :
       ...
       docker run -v $OVPN_DATA:/etc/openvpn --rm jessenich91/openvpn ovpn_genconfig -u udp://VPN.SERVERNAME.COM
       docker run -v $OVPN_DATA:/etc/openvpn --rm -it jessenich91/openvpn ovpn_initpki
   
3. Download the [docker-openvpn@.service](https://raw.githubusercontent.com/jessenich/docker-openvpn/master/init/docker-openvpn%40.service)
   file to `/etc/systemd/system`:

        curl -L https://raw.githubusercontent.com/jessenich/docker-openvpn/master/init/docker-openvpn%40.service | sudo tee /etc/systemd/system/docker-openvpn@.service
