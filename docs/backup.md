...
## Backup to Archive
    ...
    docker run -v $OVPN_DATA:/etc/openvpn --rm jessenich91/openvpn tar -cvf - -C /etc openvpn | xz > openvpn-backup.tar.xz
...
Creates an volume container named `$OVPN_DATA` to extract the data to.
    ...
    xzcat openvpn-backup.tar.xz | docker run -v $OVPN_DATA:/etc/openvpn -i jessenich91/openvpn tar -xvf - -C /etc
